...
export async function registrarPlatos(datos){
    const {nombre, descripcion, precio, restauarnteId} = datos;

    if(!nombre || !descripcion || !precio || !restauarnteId){
        throw new Error("Falta algún campo");
    }

    const platoExistente = await obtenerDB().collection(COLECCION_PLATOS).findOne({nombre});
...
export async function actualizarPlato(id,datos){
    const resultado = await obtenerDB().collection(COLECCION_PLATOS).findOne({_id: new ObjectId(id)},{$set:datos});
    return resultado.matchedCount > 0;
}

export async function eliminarPlato(id){
    const resultado = await obtenerDB().collection(COLECCION_PLATOS).findOne({_id:new ObjectId(id)});
